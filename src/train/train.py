...
from src.util.data_utils import make_supervised_data_module, make_supervised_multiple_images_data_module, DataArguments
...
class ModelArguments:
    ...
    mm_interactor_type: Optional[str] = field(default='ca_interactor')
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'mm_interactor', 'vision_tower', 'vision_resampler']
...
def train():
    ...
    if training_args.lora_enable:
        ...
        rank0_print("Adding LoRA adapters...", lora_config.target_modules)
    ...
    if model_args.tune_mm_mlp_adapter:
        ...
        for p in model.get_model().mm_interactor.parameters():
            p.requires_grad = True
    ...
    if data_args.is_multiple_images:
        data_module = make_supervised_multiple_images_data_module(tokenizer=tokenizer,
                                                               data_args=data_args)
    else:
    ...
    for name, params in model.named_parameters():
        rank0_print(name, params.requires_grad)
